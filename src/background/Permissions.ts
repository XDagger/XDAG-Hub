...
class Permissions {
  ...
  public async getPermission( origin: string, permission?: Permission | null, ): Promise<Permission | null> {
    if (permission && permission.origin !== origin) {
      throw new Error( `Provided permission has different origin from the one provided. "${permission.origin} !== ${origin}"`, );
    }
    if (permission) {
      return permission;
    }
  ...
  public async delete(origin: string, specificAccounts: XDagAddress[] = []) {
