...
/***
 * if user in restricted country, then can't open wallet . navigate to /restricted page.
 * or to normal page.
 */
...
export function useRestrictedGuard() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch("https://wallet-rpc.testnet.Xdag.io/", {
        method: "POST",
        body: JSON.stringify({
          id: 1,
          method: "Xdag_getLatestCheckpointSequenceNumber",
          jsonrpc: "2.0",
          params: [],
        }),
        headers: {
