	...
	return showPasswordDialog ? (
		<div className="absolute inset-0 pb-8 px-2.5 flex flex-col z-10">
			<PasswordInputDialog
				...
				onPasswordVerified={ async ( password ) => {await importMutation.mutateAsync( password );} }
				background
				spacing
			/>
		</div>
	) : (
		<MenuLayout title={ t( "ImportPrivateKey.ImportExistingAccount" ) } back={ accountsUrl }>
			<Formik
			...
			>
				{ ( { isSubmitting, isValid } ) => (
					<Form className="flex flex-col gap-3 pt-2.5">
						<FieldLabel txt={ t( "ImportPrivateKey.EnterPrivateKey" ) }>
							<Field
								name="privateKey"
						...
						<Button
							...
							size="tall"
							variant="primary"
							text={ t( "ImportPrivateKey.Continue" ) }
							after={ <ArrowRight16/> }
							disabled={ !isValid }
