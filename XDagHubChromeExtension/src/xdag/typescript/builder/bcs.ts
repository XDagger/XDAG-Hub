...
import { bcs } from "../types/xdag-bcs.js";
...
import type { TypeName } from "_src/xdag/bcs";
...
builder
	...
	// Keep this in sync with crates/xdag-types/src/messages.rs
	.registerEnumType( TRANSACTION_INNER, {
		 ...
		 */
		TransferObjects: { objects: [ VECTOR, ARGUMENT ], address: ARGUMENT, },
		/**
		 ...
		 */
		Publish: { modules: [ VECTOR, [ VECTOR, BCS.U8 ] ], dependencies: [ VECTOR, BCS.ADDRESS ], },
		/**
...
builder.registerType(
	...
	function encodeProgrammableTx(
		...
		return this.getTypeInterface( PROGRAMMABLE_CALL_INNER )._encodeRaw.call(
			...
			writer,
			{
				package: normalizeXDagAddress( pkg ),
				module,
				function: fun,
				type_arguments,
