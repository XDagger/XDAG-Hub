...
export class BCS
{
	 ...
	 */
	public ser( type: TypeName | StructTypeDefinition, data: any, options?: BcsWriterOptions, ): BcsWriter {
	 ...
	 * @example
	 * ```
	 * let bcs = new BCS(getXdagMoveConfig());
	 ...
	 * @example
	 * bcs.registerAddressType('address', XDAG_ADDRESS_LENGTH);
	 ...
	 * @example
	 * let { typeName, typeParams } = parseTypeName('Option<Coin<XDAG>>');
	 * // typeName: Option
	 * // typeParams: [ 'Coin<XDAG>' ]
	 *
...
export function getXdagMoveConfig(): BcsConfig {
	return {
		genericSeparators: [ "<", ">" ],
		vectorType: "vector",
		addressLength: XDAG_ADDRESS_LENGTH,
