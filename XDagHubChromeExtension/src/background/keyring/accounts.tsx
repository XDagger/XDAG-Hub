...
type AccountsPublicInfo = Record<XDagAddress, AccountPublicInfo>;

export async function getStoredAccountsPublicInfo(): Promise<AccountsPublicInfo> {
  return (await getFromLocalStorage(STORE_KEY, {})) || {};
}

export function storeAccountsPublicInfo(
  accountsPublicInfo: AccountsPublicInfo,
) {
...
export async function updateAccountsPublicInfo(
  accountsUpdates: AccountsPublicInfoUpdates,
) {
...
export async function deleteAccountsPublicInfo(
  accounts: { toDelete: XDagAddress[] } | { toKeep: XDagAddress[] },
) {
  const allAccountsPublicInfo = await getStoredAccountsPublicInfo();
  const newAccountsPublicInfo: AccountsPublicInfo = {};
  for (const [anAddress, anAccountInfo] of Object.entries(
    allAccountsPublicInfo,
  )) {
    if (
